...
public class GenericStringMatcher  extends AbstractProcessImpl implements DoubleFunctionProcess {
	...
	protected String[][][] leftResourcesPropertiesString=null;
	protected String[][][] rightResourcesPropertiesString=null;
	...
	private boolean checkData()  {
		...
		leftResources=leftNetwork.makeRDFQueryAsResourcesArray(leftResourcesQuery);
		if(leftResources==null) {
			notifyErrorMessage("<message>Left query yielded null!!</message>");
			return false;
		}
		notifyProgressMessage("<message>Query performed<br/>"+leftResourcesQuery.getQueryText()+"<br/> Yielded "+leftResources.length +" results</message>");

		
		rightResources=rightNetwork.makeRDFQueryAsResourcesArray(rightResourcesQuery);
		if(leftResources==null) {
			notifyErrorMessage("<message>Right query yielded null!!</message>");
			return false;
		}
		notifyProgressMessage("<message>Query performed<br/>"+rightResourcesQuery.getQueryText()+"<br/> Yielded "+rightResources.length +" results</message>");
	...
	private boolean prepareData() {
		...
		if(mergeProperties) {
			leftResourcesMergedString=new String[leftResources.length][];
			rightResourcesMergedString=new String[rightResources.length][];
		}
		else {
			leftResourcesPropertiesString=new String[leftResources.length][leftPropertyArray.length][];
			rightResourcesPropertiesString=new String[rightResources.length][rightPropertyArray.length][];
		}
			...
			for(int pn=0;pn<leftPropertyArray.length;pn++) {
				...
				while(striter.hasNext()) {
					...
					if(node.isLiteral()) {
						...
						strMergedSet.add(((Literal)node).getString());
				...
				leftResourcesMergedString[cn]= strMergedSet.toArray(tempSTRA);
		...
		for(int cn=0;cn<rightResources.length;cn++) {
			...
			for(int pn=0;pn<rightPropertyArray.length;pn++) {
				...
				String[] tempSTRA=new String[1];
				rightResourcesMergedString[cn]= strSet.toArray(tempSTRA);
			}
		}
	...
	public void startComputation() {
		...
		long allComps=(leftResources.length)*(rightResources.length);
		long ticklimit=10000000;
		if(allComps<100000000) ticklimit=allComps/100;
		int tick=0;
		...
		Property assertedPropery=ResourceFactory.createProperty(assertedRelationString);
			...
			*/
			if(useReference && ! hashReference) {
				...
				if(leftResourcesMergedString==null || rightResourcesMergedString==null) {
					notifyDebugMessage("<message>No strings on one of the two sides...(possibly two)</message>");
					return;
				}
				for(int lc=0;lc<leftResources.length;lc++) {
					for(int rc=0;rc<rightResources.length;rc++) {
						if(mergeProperties) {
							for(int x=0; x<leftResourcesMergedString[lc].length;x++) {
								for(int y=0; y<rightResourcesMergedString[rc].length;y++) {
									//String explanation="";
									//int confidence=0;
									//String type="";
									if(leftResourcesMergedString[lc][x]!=null && rightResourcesMergedString[rc][y]!=null) {
										String leftString=leftResourcesMergedString[lc][x];
										String rightString=rightResourcesMergedString[rc][y];
										...
										if(leftString.length()>0 && rightString.length()>0) {
											...
											if(useReference) {
												...
												refRef=processHashReference(leftString,rightString);
												if(refRef!=null) {
													refcounter++;
											...
											if(result2!=null) {
												String leftURI=leftResources[lc].getURI();
												String rightURI=rightResources[rc].getURI();
												if(!leftURI.equals(rightURI)) {
													...
													notifyDebugMessage("<message>Found alignment between:</br>" +
															"1) "+leftResources[lc]+"</br>"+
															"2) "+rightResources[rc]+"</br>"+
															"explanation: "+
												...
												if(speedy==true) {
													x=leftResourcesMergedString[lc].length;
													y=rightResourcesMergedString[rc].length;
												}
											}
											totaleval++;									
										}
						...
						if(tick>ticklimit) {
							tick=0;
							tickpercentage++;
							notifyProgressStatus(0, lc, leftResources.length);
